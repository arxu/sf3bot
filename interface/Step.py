...
def set_difficulty(frame_ratio, difficulty):
    ...
    if difficulty % 8 < 3:
        steps += [{'wait': int(10 / frame_ratio),
                  'actions': [Actions.P1_LEFT]} for i in range(3
                  - difficulty % 8)]
    else:
        steps += [{'wait': int(10 / frame_ratio),
                  'actions': [Actions.P1_RIGHT]} for i in
                  range(difficulty % 8 - 3)]
    steps += [
        {'wait': int(10 / frame_ratio), 'actions': [Actions.P1_DOWN]},
        {'wait': int(10 / frame_ratio), 'actions': [Actions.P1_DOWN]},
        {'wait': int(10 / frame_ratio), 'actions': [Actions.P1_DOWN]},
...
def start_game(frame_ratio):
    return [
        ...
        {'wait': int(60 / frame_ratio), 'actions': [Actions.P1_RIGHT]},
        {'wait': int(60 / frame_ratio), 'actions': [Actions.P1_UP]},
        {'wait': int(60 / frame_ratio), 'actions': [Actions.P1_UP]},
        {'wait': int(80 / frame_ratio), 'actions': [Actions.P1_UP,
         Actions.P1_JPUNCH]},
